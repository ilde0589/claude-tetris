...
- `game.js` — module-level mutable state (`board`, `current`, `next`, `score`, ...) reset by `init()`.
  ...
  - Visual skins: `SKINS` map (`retro`/`neon`/`pastel`/`pixel`), each with its own `colors` array (same 0–10 index space as `COLORS`, bomb included) and a `draw(context, x, y, colorIndex, size, color)` that paints only the block body — `drawBlock` looks up `currentSkin` in `SKINS` and dispatches to it, then always calls `drawBombGlyph` on top for `BOMB_TYPE` so the bomb icon renders identically in every skin. Each skin also has a `bg` (`null` for `retro`, which leaves the canvas transparent so the theme's `--board-bg` CSS var shows through; a fixed hex for the others, painted via `ctx.fillRect` at the top of `draw()`/`drawNext()` so it overrides the light/dark theme on the board/next canvases only — HUD and panel keep following the theme toggle). `applySkin(name)` mirrors `applyTheme`: sets `currentSkin`, persists to `localStorage` under `tetris-skin`, syncs `#skin-select`, and redraws immediately.
